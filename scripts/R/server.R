...
server <- function(input, output, session) {
  ...
  address_lookup_db_path <- file.path("data", "address_geoid.sqlite")
 ...
 # Session-based quota tracking assumes a 500-request limit and counts requests made during this session.
  ...
  catalog_path <- file.path("data", "census_catalog.rds")
  variable_catalog_path <- file.path("data", "census_variable_catalog.rds")
  ...
  observeEvent(input$process_btn, {
    ...
    tryCatch({
      ...
      db_path <- file.path("data", "my_app_database.sqlite")
