...
clean_addresses <- function(df) {
  ...
  lookup_db_path <- file.path("data", "address_geoid.sqlite")
  ...
  if (nrow(errors) > 0) {
    errors$error_reason <- ifelse(
      ...
      ifelse(
        ...
        ifelse(
          !lookup_db_ok,
          "Lookup database not found: data/address_geoid.sqlite",
...
attach_geoid_from_lookup <- function(df, db_path, geocode_limit = 300) {
  ...
  if (is.null(con)) {
    return(list(
      ...
      matched = 0,
      used_lookup = FALSE,
      message = "Could not open data/address_geoid.sqlite for address-to-GEOID lookup."
    ))
  }
  on.exit(dbDisconnect(con), add = TRUE)
